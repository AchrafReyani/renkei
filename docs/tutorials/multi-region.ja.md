...
> **誰がログインできるか。** 地域はそのチャネルがどの市場向けかを表すもので、ログインを自動的に塞ぐ壁ではありません。実際に日本で登録された LINE アカウントが台湾地域のチャネルに（同意画面も通って）ログインできました（2026-09-05 の検証）。ただし全地域・全ステータスで同じとは限らないので、対象とする市場のアカウントでそれぞれ試してください。拒否される場合も LINE 側の画面で起き、renkei 側の挙動は変わりません。
...
> **`line_region` が効くのは新しく認証するときだけ。** renkei は OpenID プロバイダーとして自分のセッションを持ちます。ブラウザにセッションが残っていると、2 回目以降の認可リクエストはそのセッションで応答されます。誰も再認証せず、チャネルも選ばれないため、トークンにはセッションを作ったときのチャネルが入ったままです（別の地域を要求していても同じです）。特定の地域で認証させたいときは標準の **`prompt=login`** を付けてください。renkei の `/dev` の地域リンクはそうしています。
...
- **同じ LINE プロバイダー配下の 2 チャネル**（通常はこちら）: LINE のユーザー ID はプロバイダーごとなので、同じ人は両方のチャネルで*同じ* `line:user_id` を持ちます。renkei はそれを見て **`sub` を 1 つ**に保ち、LINE アカウント行をチャネルごとに 1 行持ちます。設定は不要です。（実チャネルで確認済み・2026-09-05: 同一プロバイダーの JP チャネルと TW チャネルで同じ人がログインしたところ、`line:user_id` は同一、`sub` も 1 つで、トークンにはそれぞれの `line:region` が入りました。）
...
- [ ] 各地域の id_token に期待どおりの `line:region` と `line:channel_id` が入っている（既存セッションで応答されないよう、シークレットウィンドウか `prompt=login` で確認する）
