...
**Live check on `wrangler dev` :8787 with the real channel**: discovery / JWKS / `/dev` served
from workerd + local D1 (pinned keys, no in-memory warning), `/dev/login` → `/oidc/auth` →
`/interaction/…` → LINE; the LINE leg needs Achraf in the browser (`access.line.me` is
blocked for Claude) — see §1. Changesets staged: `renkei-storage-sqlite` minor +
...
- [ ] **Workers target, LINE leg** (Achraf): `cd examples\cloudflare-workers && pnpm dev`
      (`.dev.vars` is already there from the 2026-09-04 session, keys generated by
      `renkei init --print`), open http://localhost:8787/dev, log in with LINE. Expected:
      `/dev/callback` shows the id_token with all `line:*` claims, and the local D1
      (`.wrangler/state`) holds the identity. Tick this and ROADMAP.md's note once seen.
