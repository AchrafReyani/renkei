...
| `DATABASE_URL` | なし | `postgres://…` なら Postgres、`sqlite:./data/renkei.db` なら SQLite（Node 22.13+ 組み込みの `node:sqlite`。依存ゼロ、DB サーバー不要、1 プロセス向け。ファイルは永続ディスクに置くこと）。未設定なら**インメモリ**（再起動で全消去、複数プロセス不可。開発専用）。Cloudflare Workers ではこの変数は使わず、D1 binding `DB` がストレージになる（[ガイド](../guides/deploy-cloudflare-workers.ja.md)） |
